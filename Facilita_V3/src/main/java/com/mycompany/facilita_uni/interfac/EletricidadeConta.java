...
public class EletricidadeConta extends javax.swing.JFrame {
   ...
   try {
        ...
        // Validações (seu código de validação já está bom e foi mantido)
        if (nome.isEmpty() || data.isEmpty() || valorKwhTexto.isEmpty()) {
            javax.swing.JOptionPane.showMessageDialog(this, "Por favor, preencha todos os campos!", "Campos Obrigatórios", javax.swing.JOptionPane.WARNING_MESSAGE);
            return;
        }
        
        if (!data.matches("\\d{2}/\\d{2}")) {
            javax.swing.JOptionPane.showMessageDialog(this, "Por favor, digite a data no formato DD/MM (ex: 05/05)!", "Formato de Data Inválido", javax.swing.JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        double valorPorKwh;
        try {
            valorPorKwh = Double.parseDouble(valorKwhTexto.replace(',', '.')); // Substitui vírgula por ponto
            if (valorPorKwh <= 0) {
                throw new NumberFormatException();
            }
        } catch (NumberFormatException e) {
            javax.swing.JOptionPane.showMessageDialog(this, "Por favor, digite um valor numérico positivo para o kWh!", "Formato Inválido", javax.swing.JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        // 1. CALCULAR CORRETAMENTE OS TOTAIS
        double consumoTotalKwh = 0;
        int tempoDeUsoTotal = 0; // VARIÁVEL ADICIONADA
        List<Eletrodomestico> eletrodomesticos = catalogoCL.getEletrodomesticos();
        
        if (eletrodomesticos != null && !eletrodomesticos.isEmpty()) {
            for (Eletrodomestico eletro : eletrodomesticos) {
                consumoTotalKwh += eletro.getConsumo();
                tempoDeUsoTotal += eletro.getTempoDeUso(); // CÁLCULO ADICIONADO
            }
        }
        ...
        // 2. CRIAR O OBJETO VAZIO E USAR OS SETTERS (MÉTODO SEGURO)
        ContasLuz contaLuz = new ContasLuz();
        
        // 3. PREENCHER O OBJETO COM OS DADOS CORRETOS
        contaLuz.setNome(nome);
        contaLuz.setDataVencimento(data);
        contaLuz.setValor(valorTotal);          // Passando o double sem converter para int
        contaLuz.setKwh(consumoTotalKwh);       // Passando o consumo total correto
        contaLuz.setTempodeUso(tempoDeUsoTotal); // Passando o tempo de uso total calculado

        // ======================= FIM DAS CORREÇÕES =======================

        // Adicionar a conta à lista
        catalogoCL.adicionarContaLuz(contaLuz);
        
        // Salvar os dados atualizados
        persistencia.salvarParaArquivo(controlador, caminhoArquivo);
        
        // Mostrar mensagem de sucesso com os cálculos corretos
        javax.swing.JOptionPane.showMessageDialog(this, 
            "Conta de eletricidade salva com sucesso!\n\n" +
            "Nome: " + contaLuz.getNome() + "\n" +
            "Data de vencimento: " + contaLuz.getDataVencimento() + "\n" +
            "Valor por kWh: R$ " + String.format("%.2f", valorPorKwh) + "\n" +
            "Consumo total: " + String.format("%.2f", contaLuz.Kwh()) + " kWh\n" +
            "Tempo de uso total: " + contaLuz.TempodeUso() + "h\n" + // Informação adicionada
            "Valor total calculado: R$ " + String.format("%.2f", contaLuz.getValor()) + "\n\n" +
            "Total de contas de luz cadastradas: " + catalogoCL.getContasLuz().size(), 
            "Sucesso", 
        ...
        javax.swing.JOptionPane.showMessageDialog(this, "Erro ao processar a conta: " + e.getMessage(), "Erro", javax.swing.JOptionPane.ERROR_MESSAGE);
        e.printStackTrace();
    }
