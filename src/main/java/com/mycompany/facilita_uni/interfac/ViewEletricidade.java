...
public class ViewEletricidade extends javax.swing.JFrame {
    ...
    private void carregarContas() {
        try {
            ...
            List<ContasLuz> contas = usuario.getContasLuz(); // Assumindo que existe este método
    ...
    private JPanel criarPainelConta(ContasLuz conta, int indice) {
        ...
        JLabel valorStatus = new JLabel(status);
        valorStatus.setFont(new Font("Arial", Font.BOLD, 11));
        
        // Colorir o status
        switch (status) {
            case "VENCIDA":
                valorStatus.setForeground(new Color(220, 53, 69)); // Vermelho
                break;
            case "PRÓXIMO VENCIMENTO":
                valorStatus.setForeground(new Color(255, 193, 7)); // Amarelo/Laranja
                break;
            case "EM DIA":
                valorStatus.setForeground(new Color(40, 167, 69)); // Verde
                break;
            default:
                valorStatus.setForeground(Color.GRAY);
        }
        
        gbc.gridx = 3; gbc.gridy = 1;
        gbc.insets = new Insets(3, 5, 3, 5);
        painel.add(valorStatus, gbc);
